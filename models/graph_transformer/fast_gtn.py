...
class FastGTNs(nn.Module):
    def __init__(self, num_edge_type, w_in, num_class, num_nodes, args=None):
        ...
        for i in range(args.num_FastGTN_layers):
            if i == 0:
                fastGTNs.append(
                    FastGTN(num_edge_type, w_in, num_class, num_nodes, args)
                )
            else:
                fastGTNs.append(
                    FastGTN(num_edge_type, args.node_dim, num_class, num_nodes, args)
                )
        self.fastGTNs = nn.ModuleList(fastGTNs)
        self.linear = nn.Linear(args.node_dim, num_class)

    def forward(self, A, X, target_x, num_nodes=None):
        if num_nodes is None:
            num_nodes = self.num_nodes
        H_, Ws = self.fastGTNs[0](A, X, num_nodes=num_nodes, epoch=None)
...
class FastGTN(nn.Module):
    def __init__(
        self, num_edge_type, w_in, num_class, num_nodes, args=None, pre_trained=None
    ):
        ...
        self.Ws = []
        self.Ws.extend(
            GCNConv(in_channels=self.w_in, out_channels=self.w_out).weight
    ...
    def forward(self, A, X, num_nodes, eval=False, node_labels=None, epoch=None):
        ...
        for i in range(self.num_layers):
            if self.args.non_local:
                g = generate_non_local_graph(
                    self.args,
                    self.feat_trans_layers[i],
                    torch.stack(H).mean(dim=0),
                    A,
                    self.num_edge_type,
                    num_nodes,
                )
        ...
        for i in range(self.num_channels):
            if i == 0:
                ...
                H_ = torch.cat(
                    (
                        H_,
                        F.relu(self.args.beta * (X_[i]) + (1 - self.args.beta) * H[i]),
                    ),
                    dim=1,
                )
...
class FastGTLayer(nn.Module):
    def __init__(
        ...
        if pre_trained is not None:
            self.conv1 = FastGTConv(
                in_channels,
                out_channels,
                num_nodes,
                args=args,
                pre_trained=pre_trained.conv1,
            )
        else:
            self.conv1 = FastGTConv(in_channels, out_channels, num_nodes, args=args)
        self.args = args
        self.feat_transfrom = nn.Sequential(
            nn.Linear(args.w_in, 128), nn.ReLU(), nn.Linear(128, 64)
        )
    ...
    def forward(self, H_, A, num_nodes, epoch=None, layer=None):
        ...
        for i in range(len(result_A)):
            ...
            mat_a = torch.sparse_coo_tensor(a_edge, a_value, (num_nodes, num_nodes)).to(
                a_edge.device
            )
...
class FastGTConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, num_nodes, args=None, pre_trained=None
    ):
