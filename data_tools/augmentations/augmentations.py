...
class BoxAugmentator:
    ...
    def __call__(self, bbox: Bbox) -> Bbox:
        ...
        x, y = max(min(self.shift_dist(), 0.25), -0.25), max(
            min(self.shift_dist(), 0.25), -0.25
        )
        x, y = int(x * self.imgsz[1]), int(y * self.imgsz[0])
        if (
            bbox.x1 + x <= 0
        ...
        scale_w, scale_h = max(min(self.scale_dist(), 1 + 0.25), 1 - 0.25), max(
            min(self.scale_dist(), 1 + 0.25), 1 - 0.25
        )
