    ...
    def add_box_augementator(self, p: float, dist_type: DistType, sigma: float) -> None:
        ...
        if dist_type == DistType.UNIFORM:
            ...
            box_aug_dist_scale = Distribution(
                dist_type=dist_type, low=1 - sigma, high=1 + sigma
            )
        elif dist_type == DistType.NORMAL:
            box_aug_dist_shift = Distribution(dist_type=dist_type, mean=0, sigma=sigma)
            box_aug_dist_scale = Distribution(dist_type=dist_type, mean=1, sigma=sigma)
