    ...
    def _create_annotation(
        ...
        for anno_i, anno in enumerate(gt_dict[str_im_id]):
            ...
            mask_path = str(
                scene_root
                / "mask"
                / "{:06d}_{:06d}.png".format(int(str_im_id), int(scene_id)),
            )
            ...
            # xyz_path = (
            #    self.split_renders_path
            #    / f"crop_render_{int(scene_id):06d}_{int(str_im_id):06d}.npy"
            # )
            # xyz_path = str(scene_root / f"{int(str_im_id):06d}_{anno_i:06d}-xyz.pkl")
            ...
            # annotations["xyz_path"].append(xyz_path)
            # annotations["bbox3d_and_center"].append(self.models[obj_id - 1])
